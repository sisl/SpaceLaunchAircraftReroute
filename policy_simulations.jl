...
##          proposed simulation          ##
###########################################
function proposed(problem, policy, xArray, yArray, nameArray, xEllipse, yEllipse, weight, distNom)
	##      Setup final result vectors       ##
	###########################################
	## list of average distance traveled
	        ...
	        for j = 1.:100.
	            ...
	            for t = 1:1000
	                ...
	                if timeRemaining < 0 || timeRemaining > 81
	                	...
	                	valid_action = false
	                	for trial = 1:25
							act = action_index(policy, State([xList[t], yList[t], phiDeg, k, timeRemaining]))
	                		if CSLV_.reward(problem, State([xList[t], yList[t], phiDeg, k, timeRemaining]), problem.actionArray[act]) != -Inf
	                			valid_action = true
	                			break
	                		end
	                	end
	                	if valid_action == false
	                		println("ah shit at t=$(t)")
	                	end
	                end
